...
def render_candlestick_chart(
    ...
    if style == "Neon Candlestick":
        ...
        fg_color = "#FFFFFF"
        border_color = "#333333"
        c_inc = "#39FF14"  # Neon green
        c_dec = "#FF073A"  # Neon red
        wick_width = 1.2
        body_alpha = 0.92
    else:
        fig_bg = Colors.BG_PRIMARY
        ax_bg = Colors.BG_PRIMARY
        fg_color = Colors.TEXT_PRIMARY
        border_color = Colors.BORDER
        c_inc = "#00B050"  # Professional green
        c_dec = "#E53935"  # Professional red
        wick_width = 1.0
        body_alpha = 1.0
    ...
    price_ax = axes[0]
    _style_axis(price_ax, ax_bg, fg_color, grid=options.get("grid", True), border_color=border_color)
    ...
    # Neon glow effect — draw wider translucent bars behind bodies
    if style == "Neon Candlestick":
        glow_w = width * 1.5
        price_ax.bar(xs[increasing], closes[increasing] - opens[increasing], bottom=opens[increasing], width=glow_w, align="center", color=c_inc, alpha=0.15, zorder=1)
        price_ax.bar(xs[decreasing], closes[decreasing] - opens[decreasing], bottom=closes[decreasing], width=glow_w, align="center", color=c_dec, alpha=0.15, zorder=1)
    ...
    if show_volume:
        vol_ax = axes[1]
        _style_axis(vol_ax, ax_bg, fg_color, grid=options.get("grid", True), border_color=border_color)
    ...
    if options.get("tooltip", True):
        cursor = Cursor(price_ax, useblit=True, color=fg_color, linewidth=0.7, alpha=0.5)
        # Keep a reference to cursor so it doesn't get garbage collected
